...
public class AnswerDAO extends AbstractDAO<AnswerEntity>{
	...
	public boolean save(AnswerEntity answer) {
		...
		String sql = "INSERT INTO flow.answer (id,name,description,nextform,tag,versionid) "+
					 "VALUES ('"+answer.getId()+"','"+answer.getName()+"','"+answer.getDescription()+"','"+answer.getNextForm()+"','"+answer.getTag().getId()+"','"+answer.getVersionId().getId()+"') ";
	...
	public boolean update(AnswerEntity answer) {
		boolean result = true;
		String sql = "UPDATE flow.answer SET name='"+answer.getName()+"',description='"+answer.getDescription()+"',nextform='"+answer.getNextForm()+"',tag='"+answer.getTag().getId()+"',versionid='"+answer.getVersionId().getId()+"' "+
