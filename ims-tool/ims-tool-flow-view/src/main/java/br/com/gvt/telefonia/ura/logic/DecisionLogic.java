...
public class DecisionLogic {
	...
	public static void process(Form form,List<FluxElement> listElements, FluxElement parent) throws Exception
	{
		...
		DecisionLogic.form = form;
		DecisionLogic.listElements = listElements;
		DecisionLogic.parent = parent;
		
		if(listElements.size() > 0)
		{
			DecisionLogic.process();
		} else 
			DecisionFollowLogic.process(form, listElements, parent);
	}
	
	private static void process() throws Exception
	{
		try{
			DecisionLogic.loadDecision(
	...
	private static void loadDecision(Decision decision) throws Exception
	{
		...
		element.setForm(DecisionLogic.form.getId());
		//element.setNext(form.getNextFormDefault());
		element.setType(Decision.class.getSimpleName());
		element.setFormName(form.getName());
		element.setNextFormDefault(form.getNextFormDefault());
		
		if(DecisionLogic.listElements.size() > 0){
				if(DecisionLogic.parent != null)
					element.setParent(DecisionLogic.parent);
		} else {
		...
		DecisionLogic.loadDecisionGroup(decision.getId(),element);
	...
	private static void loadDecisionGroup(long decisionid, FluxElement elementDecision) throws Exception
	{
		...
		List<DecisionChance> listDecisionChance = null;
			...
			element.setFormName(elementDecision.getFormName());
			...
			for(DecisionChance opc : listDecisionChance)
			{
				try{
					if(!opc.getDecisiongroupchild().isEmpty())
					{
							...
							clone = (FluxElement) elementDecision.clone();
							clone.setTag(Util.tagDecisionChance(opc));
							//DecisionLogic.loadDecisionGroupFind(opc.getDecisiongroupchild(),clone);
							DecisionLogic.loadDecisionGroupFind(Long.parseLong(opc.getDecisiongroupchild()),clone);
							continue;
					} else {
							...
							clone = (FluxElement) elementDecision.clone();
							clone.setTag(Util.tagDecisionChance(opc));
							//FormLogic next = new FormLogic(opc.getNextformid(),DecisionLogic.listElements,clone);
							FormLogic next = new FormLogic(opc.getNextformid(),DecisionLogic.listElements,clone);
							next.loadFormType();
					}
				} catch(FluxException e){
